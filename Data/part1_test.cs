namespace NeuralNetworks.Data
{
    // ENTITY FRAMEWORK - PART 1 TEST DATA MAPPING
    public class part1_test
    {
        public string theta1 { get; set; }
        public string theta2 { get; set; }
        public string theta3 { get; set; }
        public string theta4 { get; set; }
        public string theta5 { get; set; }
        public string theta6 { get; set; }
        public string thetad1 { get; set; }
        public string thetad2 { get; set; }
        public string thetad3 { get; set; }
        public string thetad4 { get; set; }
        public string thetad5 { get; set; }
        public string thetad6 { get; set; }

        public string tau1 { get; set; }
        public string tau2 { get; set; }
        public string tau3 { get; set; }
        public string tau4 { get; set; }
        public string tau5 { get; set; }
        public string dm1 { get; set; }
        public string dm2 { get; set; }
        public string dm3 { get; set; }
        public string dm4 { get; set; }
        public string dm5 { get; set; }
        public string da1 { get; set; }
        public string da2 { get; set; }
        public string da3 { get; set; }
        public string da4 { get; set; }
        public string da5 { get; set; }
        public string db1 { get; set; }
        public string db2 { get; set; }
        public string db3 { get; set; }
        public string db4 { get; set; }
        public string db5 { get; set; }

        public string ANGLE_ACC_ARM { get; set; }
    }
}
